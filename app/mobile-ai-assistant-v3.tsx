...
function tearDownPcm(session: PcmSession) {
...
export default function MobileAiAssistantV3() {
  ...
  const stopAll = useCallback(() => {
    ...
    pcmRef.current = null;
    if (session) tearDownPcm(session);
  }, []);
  ...
  async function transcribe(blob: Blob, selected: Target, durationSeconds: number) {
    if (blob.size < 1000) {
      setMessage("L’enregistrement n’a pas produit de son. Réessayez en parlant au moins une seconde.");
      setStage("ready");
      return;
    }
    ...
    try {
      ...
      if (!text) throw new Error("Aucun texte n’a été reconnu. Parlez plus près du téléphone.");
      updateTranscript(text);
      setProvider(`${String(result.provider ?? "Groq Whisper")} · ${durationSeconds.toFixed(1)} s`);
      await analyse(text, selected);
    } catch (error) {
      const description = error instanceof DOMException && error.name === "AbortError"
        ? "La transcription a pris trop de temps. Réessayez avec une dictée plus courte."
  ...
  function browserDictation(selected: Target) {
    ...
    if (!Constructor) {
      setMessage("Le micro vocal n’est pas disponible. Autorisez le microphone dans Safari ou écrivez la demande ci-dessous.");
      setStage("ready");
      return;
    }
  ...
  async function startRecording() {
    ...
    try {
      ...
      if (name === "NotAllowedError" || name === "SecurityError") {
        setMessage("Microphone refusé. Dans Safari, touchez aA → Réglages du site web → Microphone → Autoriser.");
        setStage("ready");
      } else {
        browserDictation(selected);
      }
  ...
  async function stopRecording() {
    ...
    const durationSeconds = samples.length / session.sampleRate;
    if (durationSeconds < 0.45 || samples.length < session.sampleRate * 0.35) {
      setMessage("Enregistrement trop court. Parlez au moins une seconde avant d’arrêter.");
      setStage("ready");
      return;
    }
    if (audioPeak(samples) < 0.0015) {
      setMessage("Aucun son détecté. Vérifiez l’autorisation du microphone et rapprochez le téléphone.");
      setStage("ready");
      return;
    }
    const wav = encodeMonoWav(samples, session.sampleRate);
    await transcribe(wav, session.target, durationSeconds);
  }
  ...
  return <div className="mai-overlay" role="dialog" aria-modal="true">
    <section className="mai-panel">
      ...
      {stage === "choose" && <div className="mai-choices">
        <div className="mai-intro"><Mic size={28} /><strong>Que voulez-vous créer ?</strong><span>Choisissez le document, puis appuyez sur le micro et dictez.</span></div>
      ...
      {stage !== "choose" && stage !== "review" && <div className="mai-capture">
        ...
        <button className={`mai-mic ${stage === "recording" ? "recording" : ""}`} onClick={stage === "recording" ? () => void stopRecording() : () => void startRecording()} disabled={busy}>{busy ? <Loader2 size={34} className="mai-spin" /> : stage === "recording" ? <Square size={30} /> : <Mic size={38} />}</button>
        ...
        {(stage === "ready" || stage === "error") && <button className="mai-secondary" onClick={() => { targetRef.current = null; setTarget(null); setStage("choose"); setMessage(""); }}>Changer de document</button>}
      </div>}
