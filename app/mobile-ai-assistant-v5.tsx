...
function localDocument(text: string): ParsedDocument {
  ...
  const chunks = text.split(/(?:\.\s+|;\s*|\bensuite\b|\bpuis\b)/i).map((chunk) => chunk.trim()).filter((chunk) => chunk.length > 4);
  const items = chunks.map((chunk): ParsedLine | null => {
    ...
    const label = chunk
      ...
      .replace(/^[-,:]\s*/, "")
      .slice(0, 220);
    return { label: label || "Prestation dictée", description: "", quantity, unit, unit_price: unitPrice, tax_rate: tax };
  }).filter((item): item is ParsedLine => Boolean(item));

  if (!items.length) {
    items.push({ label: text.slice(0, 220) || "Prestation à compléter", description: "", quantity: 1, unit: "u", unit_price: 0, tax_rate: tax });
  }
...
function localParse(text: string, target: Target): ParsedResult {
  return target === "customer" ? localCustomer(text) : localDocument(text);
}
...
export default function MobileAiAssistantV5() {
  ...
  async function analyse(text: string, selected: Target) {
    ...
    try {
      ...
      setMessage(error instanceof DOMException && error.name === "AbortError" ? "Analyse locale utilisée pour éviter un blocage réseau." : "Analyse locale utilisée. Vérifiez les informations.");
      setStage("review");
    } finally {
  ...
  async function transcribe(blob: Blob, selected: Target, duration: number) {
    ...
    try {
      ...
      setMessage(error instanceof DOMException && error.name === "AbortError" ? "La transcription a pris trop de temps." : error instanceof Error ? error.message : "Transcription impossible.");
      setStage("error");
    } finally {
      window.clearTimeout(timeout);
    }
  ...
  async function startRecording() {
    ...
    try {
      ...
      await context.resume();
      pcmRef.current = { context, source, processor, silentGain, stream, buffers, sampleRate: context.sampleRate, target: selected };
      setStage("recording");
    } catch (error) {
  ...
  return <div className="mai-overlay" role="dialog" aria-modal="true"><section className="mai-panel">
    ...
    {stage === "review" && parsed && <div className="mai-review"><div className="mai-success"><Check size={22} /><div><strong>Informations reconnues</strong><small>Analyse terminée · à vérifier</small></div></div>{message && <div className="mai-message">{message}</div>}{customerData && <div className="mai-review-card"><strong>{customerData.company_name || [customerData.civility, customerData.last_name, customerData.first_name].filter(Boolean).join(" ") || "Client à compléter"}</strong><span>{[customerData.phone1, customerData.email1].filter(Boolean).join(" · ")}</span><small>{[customerData.line1, customerData.postal_code, customerData.city].filter(Boolean).join(", ")}</small></div>}{documentData && <div className="mai-review-card mai-lines"><strong>{documentData.title || "Document préparé"}</strong><span>{documentData.customer_hint || "Client à sélectionner"}</span>{(documentData.items ?? []).map((item, index) => <div key={`${item.label}-${index}`}><span><b>{item.label || "Ligne à compléter"}</b><small>{item.quantity ?? 1} {item.unit || "u"} · TVA {item.tax_rate ?? 20} %</small></span><strong>{euro(Number(item.quantity ?? 1) * Number(item.unit_price ?? 0))}</strong></div>)}</div>}<button className="mai-primary" onClick={apply}>Ouvrir le formulaire prérempli</button><button className="mai-secondary" onClick={() => { setParsed(null); setStage("ready"); setMessage(""); }}>Corriger la dictée</button></div>}
  </section></div>;
}
