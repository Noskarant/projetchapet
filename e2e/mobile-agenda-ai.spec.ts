...
test("crée un rendez-vous dans l’agenda depuis une demande naturelle", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: "Créer avec le micro IA" }).click();
  const assistant = page.getByRole("dialog", { name: "Créer avec l’IA" });
  await expect(assistant).toBeVisible();
  await assistant.getByRole("button", { name: "Changer de document" }).click();
  await assistant.getByRole("button", { name: /Agenda/ }).click();
