...
test("prépare un devis de peinture avec hypothèses, origine des prix et marge", async ({ page }, testInfo) => {
  ...
  await expect(editor.locator('input[value="Peinture des murs – deux couches"]')).toBeVisible();
  await expect(editor.locator('input[value="Peinture des plafonds – deux couches"]')).toBeVisible();
});
